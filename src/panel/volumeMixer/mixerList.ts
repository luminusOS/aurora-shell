 ...
 */
export const VolumeMixerList = GObject.registerClass(
  {
    Properties: {
  ...
  class VolumeMixerList extends St.BoxLayout {
    ...
    private _sync(): void {
      ...
      for (const slider of visibleSliders) {
        let group = groups.get(slider.identityKey);
        if (!group) group = [];
...
export type VolumeMixerList = InstanceType<typeof VolumeMixerList>;
