...
export const TrayIconItem = GObject.registerClass(
  class TrayIconItem extends St.Button {
    ...
    override _init(item: TrayItem, iconSize: number): void {
      ...
      this.connect('button-press-event', (_actor: St.Button, event: Clutter.Event) => {
        ...
        } else if (btn === 2 && this._trayItem.secondaryActivate) {
    ...
    private _applyIcon(): void {
      ...
      if (typeof icon === 'string') {
        ...
        const scaled = icon.scale_simple(
          this._iconSize,
          this._iconSize,
          GdkPixbuf.InterpType.BILINEAR,
        );
        this._iconWidget.gicon = scaled || icon;
      } else {
    ...
    override destroy(): void {
      ...
      if (this._trayItem.destroy) this._trayItem.destroy();
      super.destroy();
    }
  },
);

export type TrayIconItem = InstanceType<typeof TrayIconItem>;
