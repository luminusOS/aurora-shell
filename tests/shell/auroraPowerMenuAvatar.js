...
export function init() {
  ...
  Scripting.defineScriptEvent('moduleRestored', 'Power menu is restored when the module is disabled');
  Scripting.defineScriptEvent('moduleReenabled', 'Power menu avatar survives a disable-enable cycle');
}
...
function getPowerMenu() {
  ...
  if (!systemItem)
    throw new Error('SystemItem not found in quick settings grid');
...
function getAvatars(menu) {
  return menu.box
    .get_children()
    .filter(child => child.has_style_class_name(AVATAR_STYLE_CLASS));
}
...
export async function run() {
  ...
  try {
    ...
    if (menu._header.visible)
      throw new Error('Default power menu header is still visible');
    ...
    if (!menu._header.visible)
      throw new Error('Default power menu header was not restored');
...
export function script_avatarShown() { _avatarShown = true; }
export function script_moduleRestored() { _moduleRestored = true; }
export function script_moduleReenabled() { _moduleReenabled = true; }

export function finish() {
  if (!_avatarShown)
    throw new Error('Power menu avatar was not shown');
  if (!_moduleRestored)
    throw new Error('Power menu was not restored on disable');
  if (!_moduleReenabled)
    throw new Error('Power menu avatar did not survive re-enable');
}
